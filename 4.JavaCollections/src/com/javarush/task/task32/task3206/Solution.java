...
public class Solution {
    ...
    public Item getProxy(Class<?> returnClass, Class<?>... itemClass) {
        Class<?>[] interfaces = new Class[itemClass.length + 1];
        interfaces[0] = returnClass;
        if (itemClass.length > 0) {
            System.arraycopy(itemClass, 0, interfaces, 1, itemClass.length);
        }
        return (Item) Proxy.newProxyInstance(this.getClass().getClassLoader(), interfaces, new ItemInvocationHandler());
    }

    public <T extends Item> T getProxy2(Class<T> type, Class<?>... additionalInterfaces) {
